...
module.exports = function(database){
    ...
    return{
        ...
        getPublication:function(req,res,next){
            var onGetPublication = function(err,doc){
                if(err){
                    return next(new Error("Could not find publication: "+err))
                }
                if(doc){
                    res.json(200,doc)
                }else{
                    res.json(404, {
                        error:true,
                        message:req.param("pubId")+" not found"
                    });
                }

            }
            Publication.findOne({_id:req.param("pubId")}, onGetPublication)
        },
